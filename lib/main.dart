                    ...
                    children: [
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          boxShadow: [
                            BoxShadow(
                              color: nextTile.color.withOpacity(0.28),
                              blurRadius: 10,
                ...
                child: !hasPlacedFirstTile
                    ? Container(
                        ...
                        child: const Row(
                          ...
                          children: [
                            Icon(Icons.touch_app_rounded, size: 16, color: Color(0xFF4338CA)),
                            SizedBox(width: 6),
                            Text(
          ...
          Expanded(
            child: LayoutBuilder(
              builder: (context, constraints) {
                final cellWidth = (constraints.maxWidth - (gridPadding * 2) - (gridSpacing * (widget.gameBoard.cols - 1))) / widget.gameBoard.cols;
                final cellHeight = cellWidth;

                return GestureDetector(
                  behavior: HitTestBehavior.opaque,
                  onTapDown: (details) => _onBoardTapDown(details, constraints),
                  child: Stack(
                    children: [
                    GridView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final isMerging = widget.gameBoard.mergingTiles.any((mt) => mt.row == row && mt.col == col);
                        final isDisappearing = widget.gameBoard.disappearingTiles.any((dt) => dt.row == row && dt.col == col);
                    ...
                    // 떨어지는 타일들
                    ...fallingTiles.map((ft) => AnimatedBuilder(
                      animation: ft.animation,
                      builder: (context, child) {
                        final startY = -cellHeight; // 위쪽에서 시작
                        final endY = ft.targetRow * (cellHeight + gridSpacing) + gridPadding;
                        final currentY = startY + ft.animation.value * (endY - startY);

                        return Positioned(
                          left: ft.col * (cellWidth + gridSpacing) + gridPadding,
                          top: currentY,
                          width: cellWidth,
                          height: cellHeight,
                    ...
                    // 사라지는 타일들
                    ...widget.gameBoard.disappearingTiles.map((dt) => AnimatedBuilder(
                      animation: dt.opacity,
                      builder: (context, child) {
                        return Positioned(
                          left: dt.col * (cellWidth + gridSpacing) + gridPadding,
                          top: dt.row * (cellHeight + gridSpacing) + gridPadding,
                          width: cellWidth,
                          height: cellHeight,
                          child: Opacity(
                    ...
                    // 병합되는 타일들
                    ...widget.gameBoard.mergingTiles.map((mt) => AnimatedBuilder(
                      animation: mt.scale,
                      builder: (context, child) {
                        return Positioned(
                          left: mt.col * (cellWidth + gridSpacing) + gridPadding,
                          top: mt.row * (cellHeight + gridSpacing) + gridPadding,
                          width: cellWidth,
                          height: cellHeight,
                          child: Center(
                    ...
                    // 병합 이펙트(레벨 비례 링 + 파티클)
                    ...widget.gameBoard.mergeBurstEffects.map((effect) => AnimatedBuilder(
                      animation: effect.progress,
                      builder: (context, child) {
                        return Positioned(
                          left: effect.col * (cellWidth + gridSpacing) + gridPadding,
                          top: effect.row * (cellHeight + gridSpacing) + gridPadding,
                          width: cellWidth,
                          height: cellHeight,
                          child: IgnorePointer(
                    ...
                    // 점수 플로팅 텍스트
                    ...widget.gameBoard.floatingScoreEffects.map((effect) => AnimatedBuilder(
                      animation: effect.progress,
                      builder: (context, child) {
                        return Positioned(
                          left: effect.col * (cellWidth + gridSpacing) + gridPadding,
...
class MergingTile {
  ...
  MergingTile(this.row, this.col, this.tile, TickerProvider vsync, this.gameBoard) {
    controller = AnimationController(vsync: vsync, duration: const Duration(milliseconds: 360));
...
class DisappearingTile {
  ...
  DisappearingTile(this.row, this.col, this.tile, TickerProvider vsync, this.gameBoard) {
    controller = AnimationController(vsync: vsync, duration: const Duration(milliseconds: 50));
...
class MergeBurstEffect {
  ...
  MergeBurstEffect(this.row, this.col, this.tile, TickerProvider vsync, this.gameBoard)
      : intensity = (0.9 + tile.level * 0.14).clamp(0.9, 2.0).toDouble() {
    controller = AnimationController(vsync: vsync, duration: const Duration(milliseconds: 420));
          ...
          child: Transform.scale(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(
                  color: tile.color.withOpacity(0.8 * ringOpacity),
                  width: ringStroke,
                ),
                boxShadow: [
                  BoxShadow(
                    color: tile.color.withOpacity((0.28 + intensity * 0.08) * ringOpacity),
          ...
          return Center(
            child: Transform.translate(
              ...
              child: Opacity(
                ...
                child: Text(
                  '✨',
                  style: TextStyle(fontSize: sparkleSize),
                ),
              ),
            ),
          );
      ...
      for (int row = 0; row < rows; row++) {
        for (int col = 0; col < cols; col++) {
          ...
          if (row + 1 < rows &&
            ...
            disappearingTiles.add(DisappearingTile(row, col, disappearingTile, vsync, this));
            mergingTiles.add(MergingTile(row + 1, col, mergedTile, vsync, this));
            mergeBurstEffects.add(MergeBurstEffect(row + 1, col, mergedTile, vsync, this));
            floatingScoreEffects.add(FloatingScoreEffect(row + 1, col, gainedPoints, mergedTile.level, vsync, this));
            onMergeTriggered?.call(MergeEvent(row: row + 1, col: col, level: mergedTile.level, points: gainedPoints));
            points += gainedPoints;
            mergedThisStep = true;
            break verticalLoop;
          }
        ...
        for (int row = 0; row < rows; row++) {
          for (int col = 0; col < cols; col++) {
            ...
            if (col + 1 < cols &&
              ...
              board[row][col + 1] = null;
              disappearingTiles.add(DisappearingTile(row, col + 1, disappearingTile, vsync, this));
              mergingTiles.add(MergingTile(row, col, mergedTile, vsync, this));
              mergeBurstEffects.add(MergeBurstEffect(row, col, mergedTile, vsync, this));
              floatingScoreEffects.add(FloatingScoreEffect(row, col, gainedPoints, mergedTile.level, vsync, this));
              onMergeTriggered?.call(MergeEvent(row: row, col: col, level: mergedTile.level, points: gainedPoints));
              points += gainedPoints;
              mergedThisStep = true;
